...
export default function Group({ group }) {
    ...
    const submitHandelr = (e) => {
        e.preventDefault();
        data.message == '' ? '' : Inertia.visit(`/groups/${group.slug}`, {
            method: 'POST',
            data: {
            ...
            <div className="flex flex-col h-screen" >
                ...
                <div className="border-t-2 flex items-center py-2 gap-x-3 px-5 bg-[#202c33]">
                    ...
                    <form onSubmit={submitHandelr} className="w-full">
                        <input  autoFocus name="message" value={data.message} onChange={(e) => setData(e.target.name, e.target.value)} type="text" autoComplete={'true'} placeholder="Start Type . . ." className="bg-[#2a3942] rounded-lg text-white w-full form-text border-0 focus:outline-none focus:ring-0 caret-sky-500 placeholder:text-gray-300" />
                    </form>
                </div>
            </div>
        </div>
...
Group.layout = (page) => <App children={page} />;